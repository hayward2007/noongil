...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            face_vector_x = np.array([
                nose_end[0] - mid_spot[0],
                nose_end[1] - mid_spot[1],
                landmarks[4][2] - (landmarks[10][2] + landmarks[152][2])/2
            ]) * vector_length
            face_vector_x = face_vector_x / np.linalg.norm(face_vector_x)
            
            face_vector_y = np.array([
                lower_spot[0] - mid_spot[0],
                lower_spot[1] - mid_spot[1],
                landmarks[152][2] - (landmarks[10][2] + landmarks[152][2])/2
            ]) * vector_length
            face_vector_y = face_vector_y / np.linalg.norm(face_vector_y)

            # Calculate z vector using cross product (perpendicular to both x and y)
            face_vector_z = np.cross(face_vector_x, face_vector_y) * vector_length
            face_vector_z = face_vector_z / np.linalg.norm(face_vector_z)

            # Project 3D vectors to 2D using perspective projection
            projected_x = (face_vector_x[:2] / (face_vector_x[2] + focal_length)) * vector_length
            projected_x = projected_x.astype(np.int32)
            
            projected_y = (face_vector_y[:2] / (face_vector_y[2] + focal_length)) * vector_length
            projected_y = projected_y.astype(np.int32)
            
            projected_z = (face_vector_z[:2] / (face_vector_z[2] + focal_length))
